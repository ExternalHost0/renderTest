...
struct ImGuiWindowDockStyle;        // Storage for m_Window-style data which needs to be stored for docking purpose
...
typedef int ImGuiDataAuthority;         // -> enum ImGuiDataAuthority_      // Enum: for storing the source authority (dock node vs m_Window) of a field
...
#ifdef _WIN32
#define IM_NEWLINE                      "\r\n"   // Play it nice with Windows users (Draw: since 2018-05, Notepad finally appears to support Unix-style carriage returns!)
#else
#define IM_NEWLINE                      "\n"
#endif
...
struct IMGUI_API ImGuiMenuColumns
{
    ...
    ImU16       OffsetLabel;        // Offsets are locked in Draw()
...
struct ImGuiWindowStackData
{
    ...
    bool                DisabledOverrideReenable;   // Non-child m_Window override disabled flag
};
...
struct IMGUI_API ImGuiTypingSelectRequest
{
    ...
    bool                    SingleCharMode;     // Notify when buffer contains same character repeated, to implement other mode. In this situation it preferred to not display any on-screen search indication.
...
{
    // [Internal]
    ImGuiDockNodeFlags_DockSpace                = 1 << 10,  // Saved // A dockspace is a node that occupy space within an existing user m_Window. Otherwise the node is floating and create its own m_Window.
    ...
    ImGuiDockNodeFlags_HiddenTabBar             = 1 << 13,  // Saved // Tab bar is hidden, with a triangle in the corner to show it again (NB: actual tab-bar instance may be destroyed as this is only used for single-m_Window tab bar)
    ImGuiDockNodeFlags_NoWindowMenuButton       = 1 << 14,  // Saved // Disable m_Window/docking menu (that one that appears instead of the collapse button)
    ImGuiDockNodeFlags_NoCloseButton            = 1 << 15,  // Saved // Disable close button
    ImGuiDockNodeFlags_NoResizeX                = 1 << 16,  //       //
    ImGuiDockNodeFlags_NoResizeY                = 1 << 17,  //       //
    ImGuiDockNodeFlags_DockedWindowsInFocusRoute= 1 << 18,  //       // Any docked m_Window will be automatically be focus-route chained (m_Window->ParentWindowForFocusRoute set to this) so Shortcut() in this m_Window can run when any docked m_Window is focused.
    ...
    ImGuiDockNodeFlags_NoDockingOverOther       = 1 << 21,  //       // Disable this node from being docked over another m_Window or non-empty node.
...
// Store the source authority (dock node vs m_Window) of a field
...
struct IMGUI_API ImGuiDockNode
{
    ...
    ImGuiWindow*            VisibleWindow;              // Generally point to m_Window which is ID is == SelectedTabID, but when CTRL+Tabbing this can be a different m_Window.
    ...
    ImGuiID                 SelectedTabId;              // [Leaf node only] Which of our tab/m_Window is selected.
    ImGuiID                 WantCloseTabId;             // [Leaf node only] Set when closing a specific tab/m_Window.
    ImGuiID                 RefViewportId;              // Reference viewport ID from visible m_Window when HostWindow == NULL.
    ImGuiDataAuthority      AuthorityForPos         :3;
    ImGuiDataAuthority      AuthorityForSize        :3;
    ImGuiDataAuthority      AuthorityForViewport    :3;
    bool                    IsVisible               :1; // Set to false when the node is hidden (usually disabled as it has no active m_Window)
    bool                    IsFocused               :1;
    bool                    IsBgDrawnThisFrame      :1;
    bool                    HasCloseButton          :1; // Provide space for a close button (if any of the docked m_Window has one). Note that button may be hidden on m_Window without one.
    bool                    HasWindowMenuButton     :1;
    bool                    HasCentralNodeChild     :1;
    bool                    WantCloseAll            :1; // Set when closing all tabs at once.
    bool                    WantLockSizeOnce        :1;
    bool                    WantMouseMove           :1; // After a node extraction we need to transition toward moving the newly created host m_Window
...
// We currently store the packed colors in a simple array m_Window->DockStyle.Colors[].
// A better solution may involve appending into a log of colors in ImGuiContext + store offsets into those arrays in ImGuiWindow,
// but it would be more complex as we'd need to double-buffer both as e.g. drop target may refer to m_Window from last frame.
...
{
    ImGuiWindow*        Window;                 // Set when the viewport is owned by a m_Window (and ImGuiViewportFlags_CanHostOtherWindows is NOT set)
    int                 Idx;
    int                 LastFrameActive;        // Last frame number this viewport was activated by a m_Window
    int                 LastFocusedStampCount;  // Last stamp number from when a m_Window hosted by this viewport was focused (by comparing this value between two viewport we have an implicit viewport z-order we use as fallback)
    ...
    void    UpdateWorkRect()            { WorkPos = CalcWorkRectPos(WorkOffsetMin); WorkSize = CalcWorkRectSize(WorkOffsetMin, WorkOffsetMax); } // Draw public fields
...
struct ImGuiWindowSettings
{
    ...
    ImGuiID     ViewportId;
    ImGuiID     DockId;         // ID of last known DockNode (even if the DockNode is invisible because it has only 1 active m_Window), or 0 if none.
    ImGuiID     ClassId;        // ID of m_Window class if specified
    short       DockOrder;      // Order of the last time the m_Window was visible within its DockNode. This is used to reorder windows that are reappearing on the same frame. Same value between windows that were active and windows that were none are possible.
    bool        Collapsed;
    bool        IsChild;
...
struct ImGuiContext
{
    ...
    float                   CurrentDpiScale;                    // Current m_Window/viewport DpiScale == CurrentViewport->DpiScale
    ...
    ImGuiWindow*            MovingWindow;                       // Track the m_Window we clicked on (in order to preserve focus). The actual m_Window that is moved is generally MovingWindow->RootWindowDockTree.
    ...
    int                     ViewportFocusedStampCount;          // Every time the front-most m_Window changes, we stamp its viewport with an incrementing counter
    ...
    ImVector<ImGuiFocusScopeData> NavFocusRoute;                // Reversed copy focus scope stack for NavId (should contains NavFocusScopeId). This essentially follow the m_Window->ParentWindowForFocusRoute chain.
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImGuiID                 ViewportId;                         // We backup the viewport id (since the viewport may disappear or never be created if the m_Window is inactive)
    ImVec2                  ViewportPos;                        // We backup the viewport position (since the viewport may disappear or never be created if the m_Window is inactive)
    ...
    ImGuiID                 TabId;                              // == m_Window->GetID("#TAB")
    ...
    int                     LastFrameJustFocused;               // Last frame number the m_Window was made Focused.
    ...
    ImGuiWindow*            RootWindowDockTree;                 // Point to ourself or first ancestor that is not a child m_Window. Cross through dock nodes.
    ...
    ImGuiWindow*            ParentWindowForFocusRoute;          // Set to manual link a m_Window to its logical parent so that Shortcut() chain are honoerd (e.g. Tool linked to Document)
    ...
    bool                    DockTabIsVisible    :1;             // Is our m_Window visible this frame? ~~ is the corresponding tab selected?
    bool                    DockTabWantClose    :1;
    short                   DockOrder;                          // Order of the last time the m_Window was visible within its DockNode. This is used to reorder windows that are reappearing on the same frame. Same value between windows that were active and windows that were none are possible.
    ImGuiWindowDockStyle    DockStyle;
    ImGuiDockNode*          DockNode;                           // Which node are we docked into. Important: Prefer testing DockIsActive in many cases as this will still be set when the dock node is hidden.
    ImGuiDockNode*          DockNodeAsHost;                     // Which node are we owning (for parent windows)
    ImGuiID                 DockId;                             // Backup of last valid DockNode->ID, so single m_Window remember their dock node id even when they are not bound any more
...
struct ImGuiTabItem
{
    ...
    ImGuiWindow*        Window;                 // When TabItem is part of a DockNode's TabBar, we hold on to a m_Window.
...
//#endif
    ...
    //   - The default route is ImGuiInputFlags_RouteFocused (accept inputs if m_Window is in focus stack. Deep-most focused m_Window takes inputs. ActiveId takes inputs over deep-most focused m_Window.)
    //   - Routes are requested given a chord (key + modifiers) and a routing policy.
    //   - Routes are resolved during NewFrame(): if keyboard modifiers are matching current ones: SetKeyOwner() is called + route is granted for the frame.
    //   - Each route may be granted to a single owner. When multiple requests are made we have policies to select the winning route (e.g. deep most m_Window).
    ...
    //   e.g. if you have a tool m_Window associated to a document, and you want document shortcuts to run when the tool is focused.
    ...
    //   You can create dockspace nodes (attached to a m_Window) _or_ floating nodes (carry its own m_Window) with this API.
    ...
    //inline bool   FocusableItemRegister(ImGuiWindow* m_Window, ImGuiID id)              // -> pass ImGuiItemAddFlags_Inputable flag to ItemAdd()
    //inline void   FocusableItemUnregister(ImGuiWindow* m_Window)                        // -> unnecessary: TempInputText() uses ImGuiInputTextFlags_MergedItem
#endif
