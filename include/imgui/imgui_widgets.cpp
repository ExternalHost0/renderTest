...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos, ImGuiDockNode* dock_node)
{
    ...
    // Render
    //bool is_dock_menu = (m_Window->DockNodeAsHost && !m_Window->Collapsed);
    ...
    if (IsItemActive() && IsMouseDragging(0))
        StartMouseMovingWindowOrNode(window, dock_node, true); // Undock from m_Window/collapse menu button
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_visible_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        // Draw values for rendering
        scroll_ratio = ImSaturate((float)*p_scroll_v / (float)scroll_max);
        grab_v_norm = scroll_ratio * (scrollbar_size_v - grab_h_pixels) / scrollbar_size_v;

        // Draw distance to grab now that we have seek'ed and saturated
        //if (seek_absolute)
...
float ImGui::ScaleRatioFromValueT(ImGuiDataType data_type, TYPE v, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        ...
        // Awkward other cases - we need ranges of the form (-100 .. 0) to convert to (-100 .. -epsilon), not (-100 .. epsilon)
...
{
    // We other-case the extents because otherwise our logarithmic fudging can lead to "mathematically correct"
    ...
    if (is_logarithmic)
    {
        ...
        // Awkward other case - we need ranges of the form (-100 .. 0) to convert to (-100 .. -epsilon), not (-100 .. epsilon)
...
static bool InputTextFilterCharacter(ImGuiContext* ctx, unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, bool input_source_is_clipboard)
{
    ...
    if (input_source_is_clipboard == false)
    {
        ...
        // Filter private Unicode range. GLFW on OSX seems to send private characters for other keys like arrow keys (FIXME)
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        // Draw render selection flag after events have been handled, so selection highlight can be displayed during the same frame.
    ...
    if (g.ActiveId == id)
    {
        ...
        // When using 'ImGuiInputTextFlags_EnterReturnsTrue' as a other case we reapply the live buffer back to the input buffer
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // Draw NavId when clicking or when Hovering (this doesn't happen on most widgets), so navigation can be resumed with gamepad/keyboard
...
ImGuiTypingSelectRequest* ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags)
{
    ...
    // - This can be used to implement a other search mode on first character.
...
bool    ImGui::BeginTabBar(const char* str_id, ImGuiTabBarFlags flags)
{
    ...
    //if (g.NavWindow && IsWindowChildOf(g.NavWindow, m_Window, false, false))
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    // (regular tabs are permitted in a DockNode tab bar, but m_Window tabs not permitted in a non-DockNode tab bar)
    if (docked_window != NULL)
    {
        IM_ASSERT(tab_bar->Flags & ImGuiTabBarFlags_DockNode);
        tab->NameOffset = -1;
    ...
    // Transfer active id m_Window so the active id is not owned by the dock host (as StartMouseMovingWindow()
    // will only do it on the drag). This allows FocusWindow() to be more conservative in how it clears active id.
    if (held && docked_window && g.ActiveId == id && g.ActiveIdIsJustActivated)
        g.ActiveIdWindow = docked_window;

    // Drag and drop a single floating m_Window node moves it
    ...
    else if (held && !tab_appearing && IsMouseDragging(0))
    {
        ...
        // Extract a Dockable m_Window out of it's tab bar
    ...
    // That state is copied to m_Window->DockTabItemStatusFlags by our caller.
